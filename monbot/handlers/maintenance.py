...
from monbot.config import MAINT_LIST_LIMIT
from monbot.db import UserDB
from monbot.handlers.common import clean_flow_and_pending, escape_markdown_v2, format_duration, format_periods, \
  get_cb_data_val, get_tz, \
  is_maint_manager, \
  parse_date
from monbot.handlers.consts import *
from monbot.handlers.keyboards import build_hosts_keyboard, get_items_keyboard, maint_actions_kb, \
  maint_confirm_kb, maint_custom_kb
...
async def _send_confirm(update: Update, context: CallbackContext, start_ts: int, end_ts: int):
  ...
  itemid = context.user_data.get(MAINT_ITEM_KEY)
  msg_id = context.user_data.get(MAINT_MSG_ID)
  if not msg_id:
    logger.debug("_send_confirm: no MAINT_MSG_ID in context; cannot edit")
    return
  try:
    if MAINT_PENDING_ACTION_EXTEND == context.user_data[MAINT_PENDING_ACTION]:
...
async def _send_prompt(message: Message | MaybeInaccessibleMessage, context: CallbackContext, extra: str = ''):
  ...
  item_name = items_idx.item_name_by_id(itemid)
  ...
  context.user_data[MAINT_FORCE_MSG_ID] = prompt.message_id
...
async def _delete_prompt_and_reply(update: Update, context: CallbackContext):
  ...
  chat_id = update.effective_chat.id
  force_id = context.user_data.pop(MAINT_FORCE_MSG_ID, None)
  reply_id = context.user_data.pop(MAINT_REPLY_MSG_ID, None)
...
async def maint_select_host(update: Update, context: CallbackContext):
  logger.info("maint_select_host cb_data=%s", update.callback_query.data)
  await update.callback_query.answer()
  data = update.callback_query.data
  if not data.startswith(f"{CB_MAINT_HOST}:"):
    return SELECTING
  ...
  context.user_data[MAINT_HOST_KEY] = hostid
  context.user_data[MAINT_HOST_NAME_KEY] = host_name
  kb = await get_items_keyboard(hostid, context)
...
async def maint_select_item(update: Update, context: CallbackContext):
  ...
  if data.startswith(CB_MAINT_ITEM_KEYS):
    ...
    context.user_data[MAINT_ITEM_KEY] = itemid
...
async def maint_action(update: Update, context: CallbackContext):
  ...
  logger.debug(
    ...
    context.user_data.get(MAINT_FLOW_KEY),
    context.user_data.get(MAINT_PENDING_ACTION),
    context.user_data.get(MAINT_ITEM_KEY),
  )
  ...
  msvc = context.application.bot_data.get(CTX_MSVC)
  if msvc is None:
    msvc = MaintenanceService(context.application.bot_data[CTX_ZBX])
    context.application.bot_data[CTX_MSVC] = msvc
  ...
  if data.startswith(CB_MAINT_ADD):
    ...
    if active_period:
      ...
      start_ts = s
      end_ts = e + secs
      context.user_data[MAINT_PENDING_ACTION] = MAINT_PENDING_ACTION_EXTEND
    else:
      start_ts = now
      end_ts = now + secs
      context.user_data[MAINT_PENDING_ACTION] = MAINT_PENDING_ACTION_ADD_NEW

    context.user_data[MAINT_PENDING_START] = start_ts
    context.user_data[MAINT_PENDING_END] = end_ts
    context.user_data[MAINT_FLOW_KEY] = MAINT_FLOW_AWAIT_CONFIRM
  ...
  if data.startswith(CB_MAINT_NEW):
    ...
    context.user_data[MAINT_FLOW_KEY] = MAINT_FLOW_AWAIT_PERIOD
    itemid = context.user_data.get(MAINT_ITEM_KEY)
    try:
      msg_id = context.user_data.get(MAINT_MSG_ID)
      if msg_id:
        await context.bot.edit_message_text(
          ...
          parse_mode=ParseMode.MARKDOWN_V2,
          reply_markup=maint_custom_kb(itemid),
        )
    except Exception as e:
      logger.warning("edit actions message failed: %s", e)

    await _send_prompt(update.callback_query.message, context)
    context.user_data.pop(MAINT_REPLY_MSG_ID, None)
    return SELECTING

  if data.startswith(CB_MAINT_CONFIRM):
    logger.debug("maint_action: CONFIRM action=%s", context.user_data.get(MAINT_PENDING_ACTION))
    itemid = context.user_data.get(MAINT_ITEM_KEY)
    start_ts = int(context.user_data.get(MAINT_PENDING_START))
    end_ts = int(context.user_data.get(MAINT_PENDING_END))
    action = context.user_data.get(MAINT_PENDING_ACTION)
  ...
  if data == CB_MAINT_BACK_ITEMS:
    ...
    hostid = context.user_data.get(MAINT_HOST_KEY)
    host_name = context.user_data.get(MAINT_HOST_NAME_KEY, "")
    kb = await get_items_keyboard(hostid, context)
    await update.callback_query.edit_message_text(ITEM_SELECT_TITLE.format(host=host_name), reply_markup=kb)
    return SELECTING
...
async def maint_text_input(update: Update, context: CallbackContext):
  logger.info("maint_text_input flow=%s text=%s msg_id=%s", context.user_data.get(MAINT_FLOW_KEY),
              (update.message.text or "").strip(), update.message.message_id)
  flow = context.user_data.get(MAINT_FLOW_KEY)
  if flow != MAINT_FLOW_AWAIT_PERIOD:
    return
  tz = await get_tz(update, context)
  context.user_data[MAINT_REPLY_MSG_ID] = update.message.message_id
